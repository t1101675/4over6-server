...
int ip_response(int fd) {
    ...
    sprintf(msg.data, "%s 0.0.0.0 202.38.120.242 8.8.8.8 202.106.0.20 ",
            inet_ntoa(tun_addr));
...
int tun_alloc(char *dev) {
    ...
    system(buffer);
    sprintf(buffer, "ip link set dev %s mtu %u", ifr.ifr_name,
            1500 - MSG_HEADER_SIZE);
    system(buffer);
    return fd;
}
            ...
            if (events[i].data.fd == listenfd) {  // listen event
                ...
                for (; i < MAX_USER; i++) {
                    if (user_info_table[i].fd == -1) {
                        ...
                        memcpy(&(user_info_table[i].v6addr), &clientaddr,
                ...
                inet_ntop(AF_INET6, &(clientaddr.sin6_addr), str_addr,
                          sizeof(str_addr));
